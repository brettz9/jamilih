...
const MAGIC_ATTR_BUILTINS = [
  '$on', '$symbol', '$define', '$data', '$custom', '$shadow',
  '$attribute', '$text', '$document', '$DOCTYPE'
];
...
function isDOMNode (v) {
  return Boolean(
    v && typeof v === 'object' &&
    typeof (/** @type {{nodeType?: unknown}} */ (v)).nodeType === 'number'
  );
}
...
function resolveAllowable (options) {
  ...
  return {allowAny, allowable, emptyPolicy: provided && raw.length === 0};
}
 ...
 */
export const validateJamilih = (structure, options = {}) => {
  ...
  const {allowAny, allowable, emptyPolicy} = resolveAllowable(options);
  ...
  const checkExtensibleMagicKey = (key, val, path) => {
    ...
    err(
      'UNKNOWN_MAGIC_PROPERTY',
      `Unknown \`$\`-prefixed property \`${key}\` is not permitted by \`allowableOptions\``,
      path
    );
  };
    ...
    for (const k of Object.keys(obj)) {
      if (k === '$state' || isOptionKey(k)) {
        err(
          ...
          `\`${k}\` is a root-only option and cannot appear on a nested array head`,
          [...path, k]
        );
      }
    }
  ...
  const validateMapOption = (val, path) => {
    ...
    const ok = isMapLike(val) ||
      (Array.isArray(val) && (val[0] === undefined || isMapLike(val[0]))) ||
      (isPlainObject(val) && Object.hasOwn(/** @type {object} */ (val), 'root'));
    if (!ok) {
  ...
  const validateOnObject = (val, path) => {
    ...
    for (const [evt, handler] of entries) {
      const okHandler = typeof handler === 'function' ||
        (Array.isArray(handler) && typeof handler[0] === 'function');
      if (!okHandler) {
  ...
  const validateAttributesObject = (obj, path, isChildHead) => {
    for (const [key, val] of Object.entries(obj)) {
      ...
      if (isOptionKey(key)) {
        err(
          'MISPLACED_OPTIONS_OBJECT',
          `\`${key}\` is a root-only option and cannot appear ${
            isChildHead ? 'on a nested array head' : 'in an attributes object'
          }`,
          at
        );
        continue;
      ...
      if (key === '$attribute') {
        const bad = !Array.isArray(val) || val.length < 2 || val.length > 3 ||
          val.slice(1).some((s) => !isNullish(s) && typeof s !== 'string');
        if (bad) {
      ...
      if (key === '$symbol') {
        if (format === 'json') {
          ...
          const bad = !Array.isArray(val) || val.length !== 2 ||
            (typeof val[0] !== 'string' && typeof val[0] !== 'symbol') ||
            (typeof val[1] !== 'function' && !isPlainObject(val[1]));
          if (bad) {
      ...
      if (key === '$custom') {
        if (val && (typeof val === 'object' || typeof val === 'function') &&
          Object.prototype.propertyIsEnumerable.call(val, '__proto__')) {
        ...
        if (keys.length > 0 && keys.every((k) => k.startsWith('$'))) {
          ...
          for (const k of keys) {
            if (MAGIC_ATTR_BUILTINS.includes(k)) {
              err(
                'BAD_CHILD',
                `\`${k}\` produces a node and must be array-wrapped (\`[{${k}: ...}]\`), not a bare object child`,
                [...at, k]
              );
  ...
  const validateStringArg = (args, i, path) => {
    ...
    for (let k = 1; k <= need; k++) {
      ...
      if (isNullish(follow) || (typeof follow !== 'string' && typeof follow !== 'number')) {
        err(
          arg === '?' ? 'BAD_PROCESSING_INSTRUCTION' : 'BAD_SPECIAL_ARG',
          `\`${arg}\` must be followed by ${need} string argument${need > 1 ? 's' : ''}`,
          [...path, i]
        );
        return 0;
      }
  ...
  const validateArgSequence = (args, start, path, headIsChild = false) => {
    ...
    while (i < args.length) {
      ...
      if (isDOMNode(arg)) {
        ...
        validateAttributesObject(
          /** @type {Record<string, unknown>} */ (arg), at, headIsChild && i === start
        );
      } else {
  ...
  const validateOptionsObject = (obj, path) => {
    if (emptyPolicy) {
      err(
        'OPTIONS_OBJECT_NOT_ALLOWED',
        'A leading options object is not permitted (`allowableOptions: []`)',
        path
      );
      return;
    }
    for (const [key, val] of Object.entries(obj)) {
      ...
      if (!key.startsWith('$')) {
        err(
          'ATTRIBUTES_BEFORE_ELEMENT',
          'A genuine attribute may not appear on an object before an element',
          [...path, key]
        );
        continue;
      }
  ...
  const validateStructure = (struct) => {
    ...
    if (isPlainObject(first) && !isDOMNode(first)) {
      ...
      if (possibleOptions.some((k) => keys.includes(k))) {
        ...
        argStart = 1;
      } else if (keys.some((k) => FIRST_ARG_NODE_KEYS.includes(k))) {
        // A node-producing first-argument object (`#`, `$text`, ...): left for
        // `validateArgSequence` to validate in place.
  ...
  try {
    if (domConflict) {
      err(
        'OPTION_CONFLICT',
        '`allowDOM: true` conflicts with `format: "json"`; DOM nodes stay rejected',
        []
      );
    }
 ...
 * @returns {boolean}
 */
export const isValidJamilih = (structure, options) => validateJamilih(structure, options).valid;
