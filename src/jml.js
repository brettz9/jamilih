      ...
      case '#': { // Document fragment
        ...
        if (template) {
          if (Array.isArray(template)) {
            template = _getType(template[0]) === 'object'
              ? jml('template', ...template, doc.body)
              : jml('template', template, doc.body);
  ...
  for (let i = 0; i < argc; i++) {
      ...
      default: { // An element
        ...
        if (_getType(atts) === 'object' && atts.is) {
          ...
          elem = doc.createElementNS
            ? doc.createElementNS(NS_HTML, elStr, {is})
            : doc.createElement(elStr, {is});
    ...
    case 'object': { // Non-DOM-element objects indicate attribute-value pairs
      ...
      if (atts.xmlns !== undefined) { // We handle this here, as otherwise may lose events, etc.
        ...
        const replacer = typeof atts.xmlns === 'object'
          ? _replaceDefiner(atts.xmlns)
          : ' xmlns="' + atts.xmlns + '"';
        // try {
