...
function _childrenToJML (node) {
  return function (childNodeJML, i) {
    ...
    const j = Array.isArray(childNodeJML)
      ? jml(...(/** @type {JamilihArray} */ (childNodeJML)))
      : jml(childNodeJML);
    cn.replaceWith(j);
  };
}
...
*   content?: ShadowRootJamilihArrayContainer
* }} ShadowRoot
 */
...
/**
 * @typedef {null|XmlnsAttributeObject} XmlnsAttribute
 */
...
/**
 * @typedef {string|undefined|{[key: string]: string|null}} StyleAttributeObject
 */

/**
 * @typedef {{[key: string]: (this: Element, event?: Event) => void}} OnAttributeObject
 */
...
/**
 * @typedef {{extends?: string}} DefineOptions
 */

/**
 * @typedef {{[key: string]: any}} DefineMixin
 */
...
/**
 * @typedef {[DefineConstructor|DefineMixin, DefineOptions?]|[DefineConstructor, DefineMixin?, DefineOptions?]} DefineObjectArray
 */

/**
 * @typedef {DefineObjectArray|DefineConstructor|DefineMixin} DefineObject
 */
...
/**
 * @todo Add back `OnAttributeObject` here ?
 * @typedef {(string|JamilihArray|ShadowRoot|StringifiableNumber|
 *   JamilihDocumentType|JamilihDocument|XmlnsAttribute|
 *   BooleanAttribute|HandlerAttribute|DefineObject|SymbolArray|
 *   DatasetAttributeObject|StyleAttributeObject
 * )} JamilihAttValue
 */
 ...
 *   should be returned.
 * @typedef {[
 *   JamilihDoc|JamilihDoctype|JamilihTextNode|JamilihAttributeNode|JamilihOptions|ElementName|Element|Document|JamilihDocumentFragment|DocumentFragment, (ElementName|Element|DocumentFragment|Document|JamilihAttributes|JamilihChildren|JamilihAttributes|globalThis.ShadowRoot|null)?, (JamilihChildren|Element|ElementName|JamilihAttributes|globalThis.ShadowRoot|null)?, ...(ElementName|Element|JamilihAttributes|JamilihChildren|globalThis.ShadowRoot|null)[]
 * ]} JamilihArray
 */

/**
 * @typedef {[
 *   (string|Element|globalThis.ShadowRoot), (JamilihArray[]|JamilihAttributes|Element|globalThis.ShadowRoot|null)?, ...(JamilihArray[]|Element|JamilihAttributes|globalThis.ShadowRoot|null)[]
...
const jml = function jml (...args) {
    ...
    for (let [att, attVal] of Object.entries(atts)) {
      ...
      */
      case '#': { // Document fragment
        ...
        const {open, closed} = /** @type {ShadowRoot} */ (attVal);
        let {content, template} = /** @type {ShadowRoot} */ (attVal);
        ...
        /**
         * @param {DefineConstructor} [cnstrct]
        ...
        const getConstructor = (cnstrct) => {
          ...
          const baseClass = options && typeof options.extends === 'string'
            ? /** @type {typeof HTMLElement} */ (doc.createElement(options.extends).constructor)
            : customizedBuiltIn
              ? /** @type {typeof HTMLElement} */ (doc.createElement(localName).constructor)
              : window.HTMLElement;
          ...
          return cnstrct
            ? class extends baseClass {
              ...
              constructor () {
                super();
                /** @type {DefineConstructor} */ (cnstrct).call(this);
              }
            }
            : class extends baseClass {};
        };
        ...
        if (jamlihDoc.childNodes) {
          ...
          if (jamlihDoc.$DOCTYPE) {
            const dt = {$DOCTYPE: jamlihDoc.$DOCTYPE};
      ...
      default: {
        ...
        if (matchingPlugin) {
          matchingPlugin.set({
            opts,
            element: elem,
            attribute: {name: pluginName, value: attVal}
          });
          break;
        }
        attVal = checkPluginValue(elem, att, attVal, opts);
  ...
  /**
   * @param {boolean|string[]|Map<any, any>|WeakMap<any, any>|any} dataVal
   * @returns {void}
   */
  ...
  for (let i = argStart; i < argc; i++) {
    ...
    case 'string': // Strings normally indicate elements
      ...
      default: { // An element
        ...
        if (atts && _getType(atts) === 'object' && /** @type {JamilihAttributes} */ (atts).is) {
          const {is} = /** @type {JamilihAttributes} */ (atts);
          // istanbul ignore next
          elem = doc.createElementNS
            /* eslint-disable object-shorthand -- Casting */
            ? doc.createElementNS(NS_HTML, elStr, {is: /** @type {string} */ (is)})
    ...
    case 'object': { // Non-DOM-element objects indicate attribute-value pairs
      ...
      const atts = arg;
      if ('xmlns' in atts) { // We handle this here, as otherwise may lose events, etc.
      ...
      for (let j = 0; j < cl; j++) { // Go through children array container to handle elements
        ...
        default:
          ...
          if (Array.isArray(childContent)) { // Arrays representing child elements
            ...
            const newChildContent = checkPluginValue(elem, null, childContent, opts, 'children');
...
jml.toJML = function (nde, {
  ...
  if (typeof nde === 'string') {
    nde = new /** @type {import('jsdom').DOMWindow} */ (
      win
    ).DOMParser().parseFromString(nde, 'text/html'); // todo: Give option for XML once implemented and change JSDoc to allow for Element
  }
...
jml.toJMLString = function (dom, config) {
  return /** @type {string} */ (
    jml.toJML(dom, Object.assign(config || {}, {stringOutput: true}))
  );
};
...
jml.toXML = function (...args) {
  ...
  const ret = jml(...args);
  return new /** @type {import('jsdom').DOMWindow} */ (
    win
  ).XMLSerializer().serializeToString(ret);
};
...
jml.command = function (elem, symOrMap, methodName, ...args) {
  ...
  func = /** @type {Map<Element, MapCommand>|WeakMap<Element, MapCommand>} */ (
    symOrMap
  ).get(elem);
  if (!func) {
