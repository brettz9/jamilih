    ...
    for (let [att, attVal] of Object.entries(atts)) {
      ...
      */
      case '#': { // Document fragment
        ...
        const {open, closed} = /** @type {JamilihShadowRootObject} */ (attVal);
        let {content, template} = /** @type {JamilihShadowRootObject} */ (attVal);
        ...
        if (
          attVal !== null &&
          (typeof attVal === 'object' || typeof attVal === 'function') &&
          Object.prototype.propertyIsEnumerable.call(attVal, '__proto__')
        ) {
        ...
        const getConstructor = (cnstrct) => {
          ...
          const baseClass = typeof options === 'object' && typeof options.extends === 'string'
          ...
          return cnstrct
            ? class extends baseClass {
              ...
              constructor () {
                super();
                /** @type {DefineUserConstructor} */ (cnstrct).call(this);
              }
            }
            : class extends baseClass {};
        };
        ...
        window.customElements.define(
          def,
          /** @type {DefineConstructor} */ (cnstrctr),
        ...
        if (jamlihDoc.xmlDeclaration) {
          const {version, encoding, standalone} = jamlihDoc.xmlDeclaration;
          ...
          docNode.insertBefore(
            xmlDeclaration, docNode.firstChild
          );
        }
      ...
      default: {
        ...
        if (att === 'style') {
          attVal = /** @type {string} */ (
            checkPluginValue(
              elem, att, /** @type {StyleAttributeValue} */ (attVal), opts
            )
          );
          if (_isNullish(attVal)) {
            break;
  ...
  for (let i = argStart; i < argc; i++) {
    ...
    case 'null': // null always indicates a place-holder (only needed for last argument if want array returned)
      if (i === argc - 1) {
        ...
        return /** @type {U extends void ? JamilihReturn : ResolvedElement<U, W & D>} */ (nodes.length <= 1
          ? nodes[0]
    ...
    case 'array': { // Arrays or arrays of arrays indicate child nodes
      ...
      const getBadChildrenError = (childIndex) => {
        return new TypeError(
          `Bad children (parent array: ${JSON.stringify(args)}; index ${childIndex} of child: ${JSON.stringify(child)})`
        );
      };
        ...
        default:
          ...
          if (Array.isArray(childContent)) { // Arrays representing child elements
            ...
            if (
              ...
              (childHeadType !== 'string' && childHeadType !== 'object') ||
              childContent[0] === null
            ) {
              throw getBadChildrenError(j);
            }
          ...
          } else { // Single DOM element children or plugin
            let newChildContent;
 ...
 * @returns {string}
 */
export const toJMLString = function (dom, config) {
  return /** @type {string} */ (
    toJML(dom, Object.assign(config || {}, {stringOutput: true}))
  );
};
 ...
 */
export const toDOM = function (...args) { // Alias for jml()
  return jml(...args);
};
jml.toDOM = toDOM;
 ...
 * @returns {string}
 */
export const toHTML = function (...args) { // Todo: Replace this with version of jml() that directly builds a string
  const ret = jml(...args);
 ...
 * @returns {string}
 */
export const toDOMString = function (...args) { // Alias for jml.toHTML for parity with jml.toJMLString
  return toHTML(...args);
};
 ...
 * @returns {string}
 */
export const toXML = function (...args) {
  if (!win) {
    throw new Error('No window object set');
  }
  const ret = jml(...args);
 ...
 * @returns {string}
 */
export const toXMLDOMString = function (...args) { // Alias for jml.toXML for parity with jml.toJMLString
  return toXML(...args);
};
